...
class LayerList extends Sprite
{
	public function new(width:Int=250, height:Int=400)
	{
		...
		_textFormat = new TextFormat(font.fontName, Console.consoleFontSize, 0xFFFFFF);
	...
	public function set(list:IntMap<Int>)
	{
		...
		for (layer in keys)
		{
			...
			label.y = i++ * (Console.consoleFontSize + 4) + 5;
			addChild(label);
		}
		...
		_maskLabel.y = i++ * (Console.consoleFontSize + 4) + 5;
	}
