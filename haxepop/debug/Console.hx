...
class Console
{
	public static inline var consoleFontSize:Int = 20;
	...
	private function init()
	{
		...
		_sprite = new Sprite();
		var font = Assets.getFont(HXP.defaultFont);
		_format = new TextFormat(font.fontName, consoleFontSize, 0xFFFFFF);
		_back = new Bitmap();
	...
	public function enable(?trace_capture:TraceCapture, toggleKey=Key.TILDE)
	{
		...
		_entReadText.defaultTextFormat = format(1, 0xFFFFFF, "right");
#if flash
		...
		_fpsReadText.defaultTextFormat = format(1);
#if flash
		...
		_fpsInfoText0.defaultTextFormat = format(0.5, 0xAAAAAA);
		_fpsInfoText1.defaultTextFormat = format(0.5, 0xAAAAAA);
#if flash
		...
		_memReadText.defaultTextFormat = format(1);
		...
		_logReadText0.defaultTextFormat = format(1, 0xFFFFFF);
		_logReadText1.defaultTextFormat = format(big ? 1 : 0.5, 0xFFFFFF);
#if flash
		...
		_logBar = new Rectangle(8, 24, consoleFontSize, _logHeight - 8);
		_logBarGlobal = _logBar.clone();
		_logBarGlobal.y += 40;
		if (big) _logLines = Std.int(_logHeight / (consoleFontSize + 0.5));
		else _logLines = Std.int(_logHeight / (consoleFontSize/2 + 0.5));

		// The debug text.
		_sprite.addChild(_debRead);
		_debRead.addChild(_debReadText0);
		_debRead.addChild(_debReadText1);
		_debReadText0.defaultTextFormat = format(1, 0xFFFFFF);
		_debReadText1.defaultTextFormat = format(0.5, 0xFFFFFF);
#if flash
		...
		LOG.push("-- HaxePop v" + HXP.VERSION + " --");
		if (_enabled && _sprite.visible) updateLog();
	}
...
#if !js
		_memReadText.text =
			(width >= BIG_WIDTH_THRESHOLD ? "Mem: " : "") + Math.roundTo(flash.system.System.totalMemory / 1024 / 1024, 2) + "MB";
#end
	}
	...
	private function updateDebugRead()
	{
		...
		_debReadText1.setTextFormat(format(big ? 1 : 0.5));
	...
	private function format(size:Float = 1, color:Int = 0xFFFFFF, align:String = "left"):TextFormat
	{
		_format.size = size * consoleFontSize;
		_format.color = color;
		switch(align)
		{
			case "left":
