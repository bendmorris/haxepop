...
class Emitter extends Graphic
{
	 ...
	 * @param	frameHeight		Frame height.
	 */
	public function setSource(source:TileType, frameWidth:Int = 0, frameHeight:Int = 0)
	{
		_source = new Spritemap(source, frameWidth, frameHeight);
	...
	override public function update()
	{
		...
		while (p != null)
		{
			...
			if (t < p._stopTime)
			{
				...
				var py = p._y + p._oy + p._moveY * (type._backwards ? 1 - td : td) + Math.pow(t, 2)*p._gravity;
	...
	private inline function renderParticle(renderFunc:Void->Void, point:Point, camera:Camera)
	{
		// quit if there are no particles
		if (_particle == null)
		{
